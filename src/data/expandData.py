...
df.insert(21, "StarType", star_type(df["EffectiveTemperature(Teff)(K)"], df["Luminosity(L/Lo)"]))
df.insert(22, "StarPeakWavelength(nm)", [2897771.955/i for i in df["EffectiveTemperature(Teff)(K)"].to_list()])
df.insert(23, "LuminosityClass", [v[1] for v in lum_class(df["Luminosity(L/Lo)"])])
df.insert(24, "SpectralClass", [v[1] for v in spec_class(df["EffectiveTemperature(Teff)(K)"])])
